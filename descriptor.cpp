...
void Descriptor::sym_d_g2(const double eta,
                          const double Rs,
                          const double r,
                          const double rcut,
                          const double fc,
...
void Descriptor::sym_d_g4(const double zeta,
                          const double lambda,
                          const double eta,
...
void Descriptor::sym_d_g4_2(const double * const r,
                            const double * const rcut,
                            const double fcprod,
...
{
  // cosine term, all terms associated with lambda and zeta, including
  // 2^(1-zeta) i is the apex atom
...
void Descriptor::create_g4_lookup()
{
  ...
  for (size_t p = 0; p < this->name.size(); p++)
  {
    if (strcmp(this->name[p], "g4") == 0)
    {
      for (int q = 0; q < this->num_param_sets[p]; q++)
      {
        ...
        {
          std::cerr
              << "Descriptor: this model only supports integer `zeta` in `g4`."
              << std::endl;
          exit(1);
        }
  ...
  // find index of each parameter in distinct values g4_distinct_zeta,
  // g4_distinct_lambda, and g4_distinct_eta
  eps = 1e-10;
  for (size_t p = 0; p < this->name.size(); p++)
  {
    if (strcmp(this->name[p], "g4") == 0)
...
void add_distinct_value(double v, std::vector<double> & v_vec, double eps)
{
  ...
  if (already_in == false) { v_vec.push_back(v); }
}
...
int find_index(double v, std::vector<double> & v_vec, double eps)
{
  ...
  if (idx == -1)
  {
    std::cerr << "Descriptor: cannot find v = " << v << " int v_vec."
              << std::endl;
    exit(1);
  }
